...
public class CustomerMain implements Initializable {
    ...
    public void searchButtonClicked(ActionEvent event) throws IOException, SQLException {
        String searchText;
        ...
        if(searchText.length() == 0){
            ...
            searchResults = CustomerDB.getAllCustomers();
        }else{
            ...
            // If searchText is a number, search customers by index
            if(isNumber){
                int customerID = Integer.parseInt(searchText);

                // Ensure customerID is not greater than length of customer list
                if(customerID <= CustomerDB.getAllCustomers().size()){
                    foundCustomer = CustomerDB.lookupCustomer(customerID);

                    // insert found customer to Search results
                    searchResults.add(foundCustomer);

                } else {  // notify user to enter valid customer ID
            ...
            } else { // if searchText is not a number, search customer list with string
                foundCustomers = CustomerDB.lookupCustomer(searchText);
                searchResults.setAll(foundCustomers);
